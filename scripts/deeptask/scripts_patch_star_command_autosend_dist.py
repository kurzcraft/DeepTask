...
for old, new in replacements:
    if old in text:
        ...
        raise SystemExit(f'target pattern not found: {old[:80]}')
path.write_text(text)
verified = path.read_text()
print('star_autosend_replacements', changed)
print('add_to_context_sendMessage', 'if(r==="addToContext"){await o.postMessageToWebview({type:"invoke",invoke:"sendMessage"' in verified)
print('terminal_add_to_context_sendMessage', 'if(r==="terminalAddToContext"){await o.postMessageToWebview({type:"invoke",invoke:"sendMessage"' in verified)
print('session_fork_still_setChatBoxMessage', 'invoke:"setChatBoxMessage",text:`/session fork ${s}`' in verified)
