...
checks = {
    'webview_sends_terminal_payload': 'terminalOperationText:' in webview and 'terminalOperationImages:' in webview,
    ...
    'webview_optimistic_user_feedback': (
        'say:"user_feedback"' in webview
        and 'Date.now()' in webview
        and 'terminalOperationText:' in webview
    ),
    'webview_optimistic_feedback_dedupe': 'JSON.stringify' in webview and 'user_feedback' in webview,
    ...
    'extension_process_queue_completed_branch': 'else if(c||m){t.processQueuedMessages();let N="";' in extension,
