...
export function MessageList({ sessionId }: MessageListProps) {
	...
	// Find the last approval-required ask message (tool/command/completion) in the combined list
	const lastApprovalAskTs = useMemo(() => {
		...
		for (let i = combinedMessages.length - 1; i >= 0; i--) {
			...
			if (
				msg.type === "ask" &&
				(msg.ask === "tool" || msg.ask === "command" || msg.ask === "completion_result") &&
				!msg.partial
			) {
				return msg.ts
			}
	...
	const handleSuggestionClick = useCallback(
		(suggestion: SuggestionItem) => {
			...
			sendSessionEvent({
				sessionId,
				event: { type: "send_message", content: suggestion.answer },
			})
		},
		[sessionId, sendSessionEvent],
	)
