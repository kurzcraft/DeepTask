...
// Actions
function isOptimisticUserFeedback(msg: ClineMessage): boolean {
	return msg.type === "say" && msg.say === "user_feedback" && Boolean((msg.metadata as any)?.agentManagerOptimisticUserFeedback)
}

function hasMatchingUserFeedback(messages: ClineMessage[], optimistic: ClineMessage): boolean {
	return messages.some(
		(msg) =>
			msg.type === "say" &&
			msg.say === "user_feedback" &&
			msg.text === optimistic.text &&
			JSON.stringify(msg.images ?? []) === JSON.stringify(optimistic.images ?? []),
	)
}
...
export const appendSessionMessageAtom = atom(null, (get, set, payload: { sessionId: string; message: ClineMessage }) => {
	const { sessionId, message } = payload
	const messages = get(sessionMessagesAtomFamily(sessionId))
	if (hasMatchingUserFeedback(messages, message)) {
		return
	}
	set(sessionMessagesAtomFamily(sessionId), [...messages, message])
})
...
export function reconcileMessages(
	...
	const reconciled = incoming.map((incomingMsg) => {
	...
	const optimisticMessages = current.filter(
		(msg) => isOptimisticUserFeedback(msg) && !hasMatchingUserFeedback(reconciled, msg),
	)

	return [...reconciled, ...optimisticMessages]
}
