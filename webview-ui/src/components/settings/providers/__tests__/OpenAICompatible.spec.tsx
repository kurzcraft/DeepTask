...
vi.mock("../../ModelPicker", () => ({
	ModelPicker: ({ models }: any) => (
		<div data-testid="model-picker">
			Model Picker
			{models && Object.keys(models).map((modelId) => <span key={modelId}>{modelId}</span>)}
		</div>
	),
}))
...
describe("OpenAICompatible Component - includeMaxTokens checkbox", () => {
	...
	describe("Context window detection", () => {
		it("fills custom model context window from API model metadata", () => {
			const apiConfiguration: Partial<ProviderSettings> = {
			...
			const messageHandler = vi.mocked(useEvent).mock.calls.find(([eventName]) => eventName === "message")?.[1]
			...
			fireEvent.click(screen.getByText("settings:common.detect"))
		...
		it("requests API model metadata when clicking detect without cached model info", () => {
			const apiConfiguration: Partial<ProviderSettings> = {
				openAiBaseUrl: "https://api.example.com/v1",
				openAiApiKey: "test-key",
				openAiModelId: "gpt-5.5",
				openAiHeaders: { "X-Test": "1" },
				openAiCustomModelInfo: {
					contextWindow: 1,
					maxTokens: 1,
					supportsPromptCache: true,
		...
		it("keeps plain API model IDs selectable without treating missing metadata as the 128000 default", () => {
			const apiConfiguration: Partial<ProviderSettings> = {
				openAiBaseUrl: "https://api.example.com/v1",
				openAiApiKey: "test-key",
				openAiModelId: "gpt-5.5",
