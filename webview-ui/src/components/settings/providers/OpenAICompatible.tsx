...
const deeptaskOpenAiCompatibleDefaults: ModelInfo = {
	...openAiModelInfoSaneDefaults,
...
export const OpenAICompatible = ({
	...
	const pendingContextDetectionModelIdRef = useRef<string | null>(null)
	...
	// kilocode_change start: Detect context from the current OpenAI-compatible API when available.
	const applyDetectedModelInfo = useCallback(
		(modelInfo: ModelInfo) => {
			setApiConfigurationField("openAiCustomModelInfo", {
				...(apiConfiguration?.openAiCustomModelInfo || deeptaskOpenAiCompatibleDefaults),
				...(modelInfo.maxTokens && modelInfo.maxTokens > 0 ? { maxTokens: modelInfo.maxTokens } : {}),
				contextWindow: modelInfo.contextWindow || deeptaskOpenAiCompatibleDefaults.contextWindow,
			})
		},
		[apiConfiguration?.openAiCustomModelInfo, setApiConfigurationField],
	)

	const handleDetectContextWindow = useCallback(() => {
		const modelId = apiConfiguration?.openAiModelId
		const apiModelInfo = modelId ? openAiModels?.[modelId] : undefined

		if (apiModelInfo && apiModelInfo.contextWindow !== openAiModelInfoSaneDefaults.contextWindow) {
			applyDetectedModelInfo(apiModelInfo)
			return
		}

		if (apiConfiguration?.openAiBaseUrl && apiConfiguration?.openAiApiKey) {
			pendingContextDetectionModelIdRef.current = modelId || null
			vscode.postMessage({
		...
		const bundledModelInfo = modelId ? openAiNativeModels[modelId as keyof typeof openAiNativeModels] : undefined
		applyDetectedModelInfo(bundledModelInfo || deeptaskOpenAiCompatibleDefaults)
	}, [
		apiConfiguration?.openAiApiKey,
		apiConfiguration?.openAiBaseUrl,
		apiConfiguration?.openAiHeaders,
		apiConfiguration?.openAiModelId,
		applyDetectedModelInfo,
		openAiModels,
	])
	// kilocode_change end
	...
	const onMessage = useCallback((event: MessageEvent) => {
		...
		switch (message.type) {
			case "openAiModels": {
				...
				const modelInfos = Object.fromEntries(
					updatedModels.map((item) => [item, message.openAiModelInfos?.[item] || openAiModelInfoSaneDefaults]),
				)
				setOpenAiModels(modelInfos)

				const pendingContextDetectionModelId = pendingContextDetectionModelIdRef.current
				if (pendingContextDetectionModelId) {
					applyDetectedModelInfo(
						message.openAiModelInfos?.[pendingContextDetectionModelId] ||
							openAiNativeModels[pendingContextDetectionModelId as keyof typeof openAiNativeModels] ||
							deeptaskOpenAiCompatibleDefaults,
					)
					pendingContextDetectionModelIdRef.current = null
				}
				break
			}
		}
				...
				<div>
					<VSCodeTextField
						...
						<div className="flex items-center justify-between gap-2 mb-1">
							...
							<VSCodeButton appearance="secondary" onClick={handleDetectContextWindow}>
								{t("settings:common.detect")}
							</VSCodeButton>
						</div>
					</VSCodeTextField>
					<div className="text-sm text-vscode-descriptionForeground">
