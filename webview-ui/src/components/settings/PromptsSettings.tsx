...
const PromptsSettings = ({
	...
	const {
		...
		setTaskProgressFileEnabled,
	} = useExtensionState()

	const effectiveTaskProgressFileEnabled = taskProgressFileEnabled ?? true
	...
	return (
		<div>
			...
			<Section>
				<SearchableSetting
					settingId="prompts-task-progress-file"
					section="prompts"
					label="任务进度文件提示词">
					<div>
						<VSCodeCheckbox
							checked={effectiveTaskProgressFileEnabled}
							onChange={(e: Event | FormEvent<HTMLElement>) => {
								...
								setTaskProgressFileEnabled(target.checked)
								vscode.postMessage({
									type: "updateSettings",
									updatedSettings: { taskProgressFileEnabled: target.checked },
								})
							}}>
							<span className="font-medium">创建并维护任务进度清单文件</span>
						</VSCodeCheckbox>
						<div className="text-vscode-descriptionForeground text-sm mt-1 mb-3">
							开启后会在系统提示词中要求每个任务创建进度清单 Markdown 文件，并随阶段完成持续更新。
						</div>
					</div>
				</SearchableSetting>
