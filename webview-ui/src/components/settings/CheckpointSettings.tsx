...
type CheckpointSettingsProps = HTMLAttributes<HTMLDivElement> & {
	...
	taskProgressFileEnabled?: boolean
...
export const CheckpointSettings = ({
	...
	return (
		<div {...props}>
			...
			<Section>
				{/* kilocode_change start - Task progress file setting */}
				<SearchableSetting
					settingId="checkpoints-task-progress-file"
					section="checkpoints"
					label={t("settings:checkpoints.taskProgressFile.label")}>
					<VSCodeCheckbox
						checked={taskProgressFileEnabled ?? true}
						onChange={(e: any) => {
							setCachedStateField("taskProgressFileEnabled", e.target.checked)
						}}>
						<span className="font-medium">{t("settings:checkpoints.taskProgressFile.label")}</span>
					</VSCodeCheckbox>
					<div className="text-vscode-descriptionForeground text-sm mt-1">
						{t("settings:checkpoints.taskProgressFile.description")}
					</div>
				</SearchableSetting>
				{/* kilocode_change end */}
