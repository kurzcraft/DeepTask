...
async function testTerminalCommand(
	...
	try {
		...
		// VS Code exposes the same execution from executeCommand() and the start
		// event. Model that identity so duplicate read() calls cannot hide here.
		const shellExecution = {
			commandLine: { value: command },
			read: vi.fn().mockReturnValue(stream),
		}

		// Configure the mock terminal to return our execution.
		mockTerminal.shellIntegration.executeCommand.mockReturnValue(shellExecution)
		// kilocode_change end
		...
		// kilocode_change start
		expect(shellExecution.read).toHaveBeenCalledTimes(1)
		// kilocode_change end
...
describe("TerminalProcess with Bash Command Output", () => {
	...
	// kilocode_change start
	it("captures a chained compile, shell-check, and quoted Python heredoc smoke test", async () => {
		if (process.platform === "win32") {
			return
		}
		...
		const command = [
			"tmp_dir=$(mktemp -d)",
			"printf '%s\\n' '#!/usr/bin/env bash' 'exit 0' > \"$tmp_dir/run_smoke.sh\"",
			"printf '%s\\n' 'value = 1' > \"$tmp_dir/metric.py\"",
			"chmod +x \"$tmp_dir/run_smoke.sh\"",
			"python3 -m py_compile \"$tmp_dir/metric.py\"",
			"bash -n \"$tmp_dir/run_smoke.sh\"",
			"python3 - <<'PY'",
			"value = 1.0",
			"changed = 0.9",
			"assert value == 1.0",
			"assert changed < value",
			"print('syntax and metric smoke tests passed')",
			"PY",
			"status=$?",
			"rm -rf \"$tmp_dir\"",
			"exit $status",
		...
		const { capturedOutput, exitDetails } = await testTerminalCommand(
			command,
			"syntax and metric smoke tests passed\n",
		)

		expect(capturedOutput).toContain("syntax and metric smoke tests passed")
