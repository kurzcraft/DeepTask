...
const COMMIT_MESSAGE_GENERATION_TIMEOUT_MS = 90_000

async function withCommitMessageTimeout<T>(operation: Promise<T>, signal?: AbortSignal): Promise<T> {
	if (signal?.aborted) {
	...
	const timeoutPromise = new Promise<never>((_, reject) => {
		timeout = setTimeout(
			() => reject(new Error("Commit message generation timed out after 90 seconds")),
...
export class CommitMessageOrchestrator {
	...
	async generateCommitMessage(
		...
		try {
			...
			const message = await withCommitMessageTimeout(
				messageGenerator.generateMessage({
					workspacePath: request.workspacePath,
					selectedFiles: resolution.files,
					gitContext,
					abortSignal: integration.abortSignal,
					onProgress: (update) => {
