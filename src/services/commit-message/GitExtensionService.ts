...
export const MAX_COMMIT_CONTEXT_CHARS = 120_000
const MAX_DIFF_CHARS_PER_FILE = 20_000
const MAX_CHANGE_SUMMARY_FILES = 500
// kilocode_change end
...
export class GitExtensionService {
	...
	public spawnGitWithArgs(args: string[]): string {
		const result = spawnSync("git", args, {
			...
			// kilocode_change start: never let SCM summary block the extension host.
		...
		if (result.signal) {
			throw new Error(`Git command timed out or was terminated (${result.signal})`)
		}
	...
	public async getCommitContext(
		...
		try {
			...
			if (targetChanges.length > 0) {
				const summaryLines = targetChanges.slice(0, MAX_CHANGE_SUMMARY_FILES).map((change) => {
					const relativePath = path.relative(this.workspaceRoot, change.filePath)
					const scope = change.staged ? "staged" : "unstaged"
					return `${this.getReadableStatus(change.status)} (${scope}): ${relativePath}`
				})
				if (targetChanges.length > summaryLines.length) {
					summaryLines.push(`[${targetChanges.length - summaryLines.length} additional files omitted]`)
				}
