...
describe("enhancePrompt", () => {
	...
	it("retries through the normalized stream when completePrompt returns empty", async () => {
		...
		const createMessage = vi.fn().mockReturnValue({
			async *[Symbol.asyncIterator]() {
				...
				yield { type: "text", text: "fix: 修复提交总结" }
			},
		})
		...
		expect(result).toBe("fix: 修复提交总结")
	})

	it("uses reasoning-only streamed output as a last-resort response", async () => {
		const createMessage = vi.fn().mockReturnValue({
			async *[Symbol.asyncIterator]() {
				yield { type: "reasoning", text: "fix: 修复仅推理响应" }
			},
		})
		;(buildApiHandler as any).mockReturnValue({ createMessage })
