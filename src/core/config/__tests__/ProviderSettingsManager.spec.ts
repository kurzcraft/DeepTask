...
describe("ProviderSettingsManager", () => {
	...
	beforeEach(async () => {
		...
		// This is needed because new users get the default OpenAI Compatible config stored
	...
	describe("initialize", () => {
		// kilocode_change start: test updated to expect OpenAI Compatible default profile for new users
		it("should initialize OpenAI Compatible default profile without base URL or API key", async () => {
			...
			const defaultConfig = storedConfig.apiConfigs.default

			expect(defaultConfig.apiProvider).toBe("openai")
			expect(defaultConfig.openAiModelId).toBe("gpt-5.5")
			expect(defaultConfig.openAiStreamingEnabled).toBe(true)
			expect(defaultConfig.openAiR1FormatEnabled).toBe(true)
			expect(defaultConfig.openAiCustomModelInfo).toEqual(
			...
			expect(defaultConfig.includeMaxTokens).toBe(true)
			expect(defaultConfig.openAiBaseUrl).toBeUndefined()
			expect(defaultConfig.openAiApiKey).toBeUndefined()
		})
		it("should migrate the previous Deeptask OpenAI default profile to the current default", async () => {
			...
			expect(defaultConfig.id).toBe("legacy-openai-default-id")
			expect(defaultConfig.apiProvider).toBe("openai")
			expect(defaultConfig.openAiModelId).toBe("gpt-5.5")
			expect(defaultConfig.openAiR1FormatEnabled).toBe(true)
			expect(defaultConfig.openAiCustomModelInfo).toEqual(
		...
		it("should migrate the old bundled Kilocode default profile to OpenAI Compatible", async () => {
			mockSecrets.get.mockResolvedValue(
				JSON.stringify({
					currentApiConfigName: "default",
					apiConfigs: {
			...
			expect(defaultConfig.id).toBe("legacy-default-id")
			expect(defaultConfig.apiProvider).toBe("openai")
			expect(defaultConfig.openAiModelId).toBe("gpt-5.5")
			expect(defaultConfig.openAiR1FormatEnabled).toBe(true)
			expect(defaultConfig.openAiCustomModelInfo).toEqual(
			...
			expect(defaultConfig.openAiBaseUrl).toBeUndefined()
			expect(defaultConfig.openAiApiKey).toBeUndefined()
			expect(defaultConfig.kilocodeModel).toBeUndefined()
		})
		// kilocode_change end
