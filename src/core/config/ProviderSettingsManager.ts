...
// kilocode_change start: Deeptask non-sensitive first-run provider defaults.
export const DEEPTASK_DEFAULT_OPENAI_MODEL_ID = "gpt-5.5"

export const DEEPTASK_DEFAULT_OPENAI_CUSTOM_MODEL_INFO = {
	maxTokens: -1,
...
export class ProviderSettingsManager {
	...
	// kilocode_change start: Seed new installs with an OpenAI-compatible profile, without endpoint or key.
	...
	// kilocode_change start: Migrate the old bundled Deeptask default to OpenAI Compatible.
	private isLegacyBundledKilocodeDefaultProfile(apiConfig: ProviderSettingsWithId | undefined): boolean {
		return (
			apiConfig?.apiProvider === "kilocode" &&
			(apiConfig.kilocodeModel === "minimax/minimax-m2.1:free" ||
				apiConfig.kilocodeModel === "minimax-m2.1:free") &&
			!apiConfig.kilocodeToken
		)
	}

	private isLegacyDeeptaskOpenAiDefaultProfile(apiConfig: ProviderSettingsWithId | undefined): boolean {
		return (
			apiConfig?.apiProvider === "openai" &&
			apiConfig.openAiModelId === "gpt-4o" &&
			!apiConfig.openAiBaseUrl &&
			!apiConfig.openAiApiKey
		)
	}

	private getDefaultOpenAiCompatibleProfile(id?: string): ProviderSettingsWithId {
		return {
			...this.defaultProviderProfiles.apiConfigs.default,
			id: id || this.defaultProviderProfiles.apiConfigs.default.id,
		}
	}
	// kilocode_change end
	...
	async init_runMigrations() {
		try {
			return await this.lock(async () => {
				...
				// kilocode_change start: Upgrade old Deeptask defaults without touching real user profiles.
				const defaultProfile = providerProfiles.apiConfigs.default
				if (
					this.isLegacyBundledKilocodeDefaultProfile(defaultProfile) ||
					this.isLegacyDeeptaskOpenAiDefaultProfile(defaultProfile)
				) {
					providerProfiles.apiConfigs.default = this.getDefaultOpenAiCompatibleProfile(defaultProfile.id)
					isDirty = true
				}
				// kilocode_change end
