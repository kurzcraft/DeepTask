...
describe("ClineProvider", () => {
	...
	test("condenseTaskContext clears frontend condensing state when condensation fails", async () => {
		const mockTask = new Task(defaultTaskOptions) as any
		mockTask.taskId = "task-condense-fails"
		mockTask.condenseContext = vi.fn().mockRejectedValue(new Error("cancelled"))
		await provider.addClineToStack(mockTask)
		const postStateToWebviewSpy = vi.spyOn(provider, "postStateToWebview").mockResolvedValue(undefined)
		const postMessageToWebviewSpy = vi.spyOn(provider, "postMessageToWebview").mockResolvedValue(undefined)

		await provider.condenseTaskContext("task-condense-fails")

		expect(postStateToWebviewSpy).toHaveBeenCalled()
		expect(postMessageToWebviewSpy).toHaveBeenCalledWith({
			type: "condenseTaskContextResponse",
			text: "task-condense-fails",
		})
	})
	...
	// kilocode_change start
	test("initializes seeded OpenAI Compatible profile into active state", async () => {
		await vi.waitFor(async () => {
			const state = await provider.getState()
			expect(state.currentApiConfigName).toBe("default")
			expect(state.apiConfiguration.apiProvider).toBe("openai")
			expect(state.apiConfiguration.openAiModelId).toBe("gpt-5.5")
			expect(state.apiConfiguration.openAiR1FormatEnabled).toBe(true)
			expect(state.apiConfiguration.openAiCustomModelInfo).toEqual(
				expect.objectContaining({ contextWindow: 256000, supportsPromptCache: true }),
			)
			expect(state.apiConfiguration.openAiBaseUrl).toBeUndefined()
			expect(state.apiConfiguration.openAiApiKey).toBeUndefined()
		})
	})

	test("syncs default profile metadata even when provider settings already exist", async () => {
		await provider.contextProxy.setProviderSettings({
			apiProvider: "openai",
			openAiModelId: "gpt-5.5",
			openAiStreamingEnabled: true,
			openAiR1FormatEnabled: true,
			openAiCustomModelInfo: { contextWindow: 256000, supportsPromptCache: true },
	...
	test("refreshes context secrets after seeding the default provider profile", async () => {
		const refreshSecretsSpy = vi.spyOn(provider.contextProxy, "refreshSecrets")

		const state = await provider.getState()

		expect(refreshSecretsSpy).toHaveBeenCalled()
		expect(state.currentApiConfigName).toBe("default")
		expect(state.apiConfiguration.apiProvider).toBe("openai")
		expect(state.apiConfiguration.openAiModelId).toBe("gpt-5.5")
		expect(state.apiConfiguration.openAiR1FormatEnabled).toBe(true)
	...
	test("migrates legacy bundled Kilocode active state to OpenAI Compatible", async () => {
		...
		await provider.providerSettingsManager.import({
			currentApiConfigName: "default",
			apiConfigs: {
				default: {
					id: "legacy-default-id",
			...
			modeApiConfigs: { code: "legacy-default-id" },
			migrations: {
		...
		expect(state.apiConfiguration.kilocodeModel).toBeUndefined()
		expect(state.apiConfiguration.openAiBaseUrl).toBeUndefined()
		expect(state.apiConfiguration.openAiApiKey).toBeUndefined()
	})

	test("migrates previous Deeptask OpenAI active state to current OpenAI Compatible defaults", async () => {
		await provider.contextProxy.setValues({
			currentApiConfigName: "default",
			listApiConfigMeta: [
				{
					name: "default",
					id: "legacy-openai-default-id",
					apiProvider: "openai",
					modelId: "gpt-4o",
				},
			],
		...
		await provider.providerSettingsManager.import({
			...
			modeApiConfigs: { code: "legacy-openai-default-id" },
			migrations: {
				rateLimitSecondsMigrated: true,
				diffSettingsMigrated: true,
				openAiHeadersMigrated: true,
	...
	test("syncs Deeptask non-sensitive global defaults into active state", async () => {
		await provider.contextProxy.setValue("allowedCommands", ["git log", "git diff", "git show"])

		const state = await provider.getState()

		expect(state.allowedCommands).toEqual(["git log", "git diff", "git show", "* "])
		...
		expect(state.alwaysAllowWriteProtected).toBe(true)
		...
		expect(state.terminalShellIntegrationTimeout).toBe(5000)
		expect(state.terminalOutputCharacterLimit).toBe(50000)
		expect(state.terminalOutputLineLimit).toBe(500)
		expect(state.terminalCompressProgressBar).toBe(true)
		expect(state.browserToolEnabled).toBe(false)
		expect(state.enableCheckpoints).toBe(false)
	})

	test("syncs completed terminal retention defaults to the runtime registry on state hydration", async () => {
		TerminalRegistry.setCompletedTerminalLimitEnabled(false)
		TerminalRegistry.setCompletedTerminalLimit(99)

		await provider.getState()

		expect(TerminalRegistry.getCompletedTerminalLimitEnabled()).toBe(true)
		expect(TerminalRegistry.getCompletedTerminalLimit()).toBe(3)
	})
	// kilocode_change end
