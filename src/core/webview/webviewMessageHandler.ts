...
export const webviewMessageHandler = async (
	...
	const findMessageIndices = (messageTs: number, currentCline: any) => {
		...
		const apiConversationHistory = Array.isArray(currentCline.apiConversationHistory)
			...
			: []
		const allApiMatches = apiConversationHistory
	...
	const handleDeleteMessageConfirm = async (messageTs: number, restoreCheckpoint?: boolean): Promise<void> => {
		...
		if (messageIndex === -1) {
			// kilocode_change start
			// Stale delete confirmations can arrive after a resend/interrupt rewrote the
			// visible timeline. Treat them as already obsolete instead of surfacing an
			// error that leaves the user in a confusing duplicate-message state.
	...
	const handleEditMessageConfirm = async (
		...
		if (messageIndex === -1) {
			...
			// A second inline resend can arrive after the first one already rewound this
			// timestamp. Keep that explicit retry, but continue ignoring delayed confirmation
			// dialog callbacks because they are stale UI events rather than new submissions.
			if (isInlineResend) {
		...
		try {
			// kilocode_change start
			// Editing/resending rewinds conversation state. Any old queued message or ask
			// now belongs to the pre-rewind tail and must not block, preempt, or consume
			// the edited prompt after the history has been restarted.
			currentCline.messageQueueService.clear()
			currentCline.clearStaleWebviewAskResponse()
			// kilocode_change end
			...
			// For non-checkpoint edits, keep only the context before the edited message.
			// The edited message itself and every later message are discarded so stale
			// follow-up context cannot leak into the restarted model request.
			const deleteFromMessageIndex = messageIndex
	...
	switch (message.type) {
		...
		case "parallel.switchWorkspace": {
			...
			// kilocode_change start: bind-or-reuse the conversation, then re-parent it so
			// the left rail always groups this session under the switched workspace.
			if (currentTask) {
				await manager.syncSessionWorkspace(currentTask.taskId, nextPath)
			}
			// kilocode_change end
		...
		case "askResponse":
			{
				...
				// askResponse that the next ask consumes immediately, graying out controls
				// and stalling reasoning. Soft completion leaves a green completion_result
				...
				const hasPendingAsk =
					pendingAskTs !== undefined || (message.askTs === undefined && task?.hasPendingWebviewAskResponse())
				const isAskResponseForCurrentAsk = message.askTs === undefined || message.askTs === pendingAskTs
				const pendingAskMessage =
					pendingAskTs === undefined
						? undefined
						: (task?.findMessageByTimestamp?.(pendingAskTs) ??
							task?.clineMessages?.find((clineMessage) => clineMessage.ts === pendingAskTs))
				const isCompletionResultPendingAsk =
					hasPendingAsk &&
				...
				const isCancelledStreamingContinuation =
					message.askResponse === "messageResponse" && hasMessagePayload && isCancellingOrAbandoned

				// Home / history list screen can still post askResponse if the webview had
				// stale sendingDisabled state. Without a current task those messages were
				// cleared and dropped, so the first typed instruction never started work.
				...
				if (task && isLiveStreamingUserInterrupt) {
					// Mid-stream user text is preemptive: park it for rehydrate, then cancel
					// the live HTTP stream so cancelTask() rebuilds the task and consumes it.
					// Do NOT use isTaskLoopActive alone: command/tool waits keep the loop flag
					// true and must answer the current ask/terminal path instead of queueing.
					...
					// continuation, not as a response to that dying ask.
					task.clearStaleWebviewAskResponse()
					task.messageQueueService.clear()
					provider.setPendingCancelledTaskContinuation?.(resolved.text ?? "", resolved.images)
					await provider.postStateToWebview()
					...
					(hasMessagePayload ||
						...
						message.askResponse === "noButtonClicked")
				) {
					// kilocode_change start
					// Stale Run/Continue buttons can arrive after the original ask already
					// settled. If the shell is still live, treat button clicks as terminal
					// wake/abort instead of clearing UI into a dead state.
					task.clearStaleWebviewAskResponse()
					task.messageQueueService.clear()
					void task.handleTerminalOperation(
						message.askResponse === "noButtonClicked" ? "abort" : "continue",
					...
					await provider.postStateToWebview()
					// kilocode_change end
				} else if (task && isContinueWithoutPendingAsk) {
					// kilocode_change start
					// A stale Continue click can arrive after the message it referred to was
					// deleted or after cancellation rehydrated the task. Do not clear the UI
					// into a disabled state without starting work; resume with an explicit
					...
					// kilocode_change start
					// Typed user messages must never be dropped. If there is no current ask,
					// no terminal process, and no special completion/cancel state, treat the
					// text as a fresh continuation so sending while the UI is briefly busy
					...
					// kilocode_change start
					// Empty button clicks without a live ask or terminal process still need to
		...
		case "updateSettings":
			if (message.updatedSettings) {
				for (const [key, value] of Object.entries(message.updatedSettings)) {
					...
					} else if (key === "terminalCompletedTerminalLimitEnabled") {
						// kilocode_change start
						if (value !== undefined) {
							TerminalRegistry.setCompletedTerminalLimitEnabled(value as boolean)
						}
						// kilocode_change end
					} else if (key === "terminalCompletedTerminalLimit") {
						// kilocode_change start
						if (value !== undefined) {
							TerminalRegistry.setCompletedTerminalLimit(value as number)
						}
						// kilocode_change end
		...
		case "terminalOperation":
			if (message.terminalOperation) {
				// kilocode_change start
				// After a command finishes, the webview may still emit terminalOperation for a
				// stale command_output row. If there is no live terminal process:
		...
		case "requestOpenAiModels":
			if (message?.values?.baseUrl && message?.values?.apiKey) {
				...
				provider.postMessageToWebview({
					type: "openAiModels",
					openAiModels,
					openAiModelInfos: openAiModels.modelInfos,
				})
			}
	...
	case "submitEditedMessage": {
		...
		case "queueMessage": {
			// kilocode_change start
			// The visible queue transport is disabled. Treat any legacy queueMessage as
			// immediate user feedback and never retain it in MessageQueueService. Preserve
			// the same anti-stall priority as askResponse: live streams must be cancelled
			// before rehydrating a parked continuation, and terminal wakes must not be
			...
			if (task) {
				const hasMessagePayload = !!(resolved.text?.trim() || resolved.images?.length)
				const isCancellingOrAbandoned = task.abortReason === "user_cancelled" || task.abandoned === true
				// Match askResponse: only cancel on live HTTP streaming, not tool/command waits.
				const isActivelyStreaming = task.isStreaming === true && !isCancellingOrAbandoned
				const isLiveStreamingUserInterrupt = hasMessagePayload && isActivelyStreaming
				const isCancelledContinuation = hasMessagePayload && isCancellingOrAbandoned

				task.messageQueueService.clear()

				if (isLiveStreamingUserInterrupt) {
					task.clearStaleWebviewAskResponse()
					provider.setPendingCancelledTaskContinuation?.(resolved.text ?? "", resolved.images)
					void provider.cancelTask()
					await provider.postStateToWebview()
				} else if (isCancelledContinuation) {
					task.clearStaleWebviewAskResponse()
					provider.setPendingCancelledTaskContinuation?.(resolved.text ?? "", resolved.images)
					await provider.postStateToWebview()
				} else if (task.terminalProcess && hasMessagePayload) {
					task.clearStaleWebviewAskResponse()
					void task.handleTerminalOperation("continue", resolved.text, resolved.images)
					await provider.postStateToWebview()
				} else if (task.hasPendingWebviewAskResponse()) {
					task.handleWebviewAskResponse("messageResponse", resolved.text, resolved.images)
				} else if (hasMessagePayload) {
					// Legacy queue transports can still be emitted by older UI or automation.
		...
		case "removeQueuedMessage": {
			// kilocode_change start
			provider.getCurrentTask()?.messageQueueService.clear()
			// kilocode_change end
			break
		}
