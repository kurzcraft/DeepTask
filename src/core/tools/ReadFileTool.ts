...
const DEFAULT_MAX_READ_FILE_LINE_CHARS = 20_000
const TRUNCATED_LINE_NOTICE_PREFIX = "[Line truncated: omitted"

function truncateLongLines(
	content: string,
	maxLineChars = DEFAULT_MAX_READ_FILE_LINE_CHARS,
): { content: string; truncatedLineCount: number } {
	if (maxLineChars <= 0 || content.length <= maxLineChars) {
	...
	const truncatedLines = lines.map((line) => {
		...
		truncatedLineCount++
		const omittedChars = line.length - maxLineChars
		return `${line.slice(0, maxLineChars)} ${TRUNCATED_LINE_NOTICE_PREFIX} ${omittedChars} chars to protect context window]`
	})

	return { content: truncatedLines.join("\n"), truncatedLineCount }
}

function buildLongLineNotice(
	...
	return `Truncated ${truncatedLineCount} overlong line${truncatedLineCount === 1 ? "" : "s"} to ${maxLineChars} characters to prevent context overflow. Use shell tools such as head, tail, sed, awk, or grep to inspect the omitted portions selectively.`
}
// kilocode_change end
				...
				try {
					...
					if (isBinary) {
						...
						if (supportedBinaryFormats && supportedBinaryFormats.includes(fileExtension)) {
							...
							try {
								const rawContent = await extractTextFromFile(fullPath)
								const { content, truncatedLineCount } = truncateLongLines(rawContent)
								const longLineNotice = buildLongLineNotice(truncatedLineCount)
								...
								updateFileResult(relPath, {
									xmlContent:
										lineCount > 0
											? `<file><path>${relPath}</path>\n<content${lineRangeAttr}>\n${numberedContent}</content>\n${longLineNotice ? `<notice>${longLineNotice}</notice>\n` : ""}</file>`
											: `<file><path>${relPath}</path>\n<content/><notice>File is empty</notice>\n</file>`,
									nativeContent:
										lineCount > 0
											? `File: ${relPath}\nLines 1-${lineCount}:\n${numberedContent}${longLineNotice ? `\n\nNote: ${longLineNotice}` : ""}`
											: `File: ${relPath}\nNote: File is empty`,
								})
								continue
							} catch (error) {
					...
					if (fileResult.lineRanges && fileResult.lineRanges.length > 0) {
						...
						for (const range of fileResult.lineRanges) {
							...
							const { content: safeContent, truncatedLineCount } = truncateLongLines(rawContent)
							const longLineNotice = buildLongLineNotice(truncatedLineCount)
							const content = addLineNumbers(safeContent, range.start)
							const lineRangeAttr = ` lines="${range.start}-${range.end}"`
							rangeResults.push(
								`<content${lineRangeAttr}>\n${content}</content>${longLineNotice ? `\n<notice>${longLineNotice}</notice>` : ""}`,
							)
							nativeRangeResults.push(
								`Lines ${range.start}-${range.end}:\n${content}${longLineNotice ? `\n\nNote: ${longLineNotice}` : ""}`,
							)
						}
					...
					if (maxReadFileLine > 0 && totalLines > maxReadFileLine) {
						const rawContent = await readLines(fullPath, maxReadFileLine - 1, 0)
						const { content: safeContent, truncatedLineCount } = truncateLongLines(rawContent)
						const longLineNotice = buildLongLineNotice(truncatedLineCount)
						const content = addLineNumbers(safeContent)
					...
					if (safeReadBudget <= 0) {
						...
						const { content: safeContent, truncatedLineCount } = truncateLongLines(result.content)
						const longLineNotice = buildLongLineNotice(truncatedLineCount)
						content = addLineNumbers(safeContent)
						...
						if (!result.complete) {
							...
							const notices = [notice, longLineNotice].filter(Boolean).join(" ")
							...
							if (longLineNotice) {
								xmlInfo += `<notice>${longLineNotice}</notice>\n`
							}
