...
describe("read_file tool with maxReadFileLine setting", () => {
	...
	describe("when maxReadFileLine is negative", () => {
		...
		it("should truncate overlong single lines when reading the entire file", async () => {
			const longLine = "a".repeat(20_050)
			mockInputContent = longLine
			mockReadFileWithTokenBudget.mockResolvedValueOnce({
				content: longLine,
				tokenCount: longLine.length / 4,
				lineCount: 1,
				complete: true,
			})

			const result = await executeReadFileTool({}, { maxReadFileLine: -1, totalLines: 1 })

			expect(result).toContain(`File: ${testFilePath}`)
			expect(result).toContain("Lines 1-1:")
			expect(result).toContain("[Line truncated: omitted 50 chars to protect context window]")
			expect(result).toContain("Note: Truncated 1 overlong line to 20000 characters")
			expect(result).not.toContain("a".repeat(20_050))
		})
...
80--90 | function bar() {`
	...
	describe("with range parameters", () => {
		...
		it("should truncate overlong single lines inside requested ranges", async () => {
			const longLine = "b".repeat(20_010)
			...
			expect(rangeResult).toContain(`File: ${testFilePath}`)
			expect(rangeResult).toContain("Lines 7-7:")
			expect(rangeResult).toContain("[Line truncated: omitted 10 chars to protect context window]")
			expect(rangeResult).toContain("Note: Truncated 1 overlong line to 20000 characters")
			expect(rangeResult).not.toContain("b".repeat(20_010))
		})
	})
})
