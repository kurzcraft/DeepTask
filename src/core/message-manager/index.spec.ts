...
describe("MessageManager", () => {
	...
	describe("Race condition handling", () => {
		it("should remove an orphaned tool_use after rewind instead of replaying invalid protocol history", async () => {
			mockTask.clineMessages = [
				{ ts: 50, say: "user", text: "Initial request" },
				{ ts: 100, say: "user_feedback", text: "resend" },
			]
			...
			mockTask.apiConversationHistory = [
				...
				{ ts: 300, role: "user", content: [{ type: "tool_result", tool_use_id: "tool_1", content: "ok" }] },
			]

			await manager.rewindToTimestamp(100)

			const apiCall = mockTask.overwriteApiConversationHistory.mock.calls[0][0]
			expect(apiCall).toEqual([{ ts: 50, role: "user", content: [{ type: "text", text: "Initial request" }] }])
		})
		...
		it("should preserve assistant message when clineMessage timestamp is earlier due to async execution", async () => {
			...
			mockTask.apiConversationHistory = [
				{ ts: 50, role: "user", content: [{ type: "text", text: "Initial request" }] },
				{
					...
					role: "assistant",
					content: [{ type: "text", text: "Assistant response emitted after UI feedback" }],
				},
				{
					ts: 300,
					role: "user",
					content: [{ type: "text", text: "tell me a joke 3" }],
				},
			]
			...
			// and use that as the actual boundary, preserving the assistant text at ts=200.
			const apiCall = mockTask.overwriteApiConversationHistory.mock.calls[0][0]
			expect(apiCall).toHaveLength(2)
