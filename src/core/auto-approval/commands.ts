...
export function containsDangerousSubstitution(source: string): boolean {
	...
	const shellInterpretedSource = stripHeredocBodiesForSafetyScan(source)
	const expansionSource = maskSingleQuotedSegmentsForSafetyScan(shellInterpretedSource)
	const shellSyntaxSource = maskQuotedSegmentsForShellSyntaxScan(shellInterpretedSource)
	// kilocode_change end
	...
	// ${var@A} - Assignment statement
	// ${var@a} - Attribute flags
	const dangerousParameterExpansion = /\$\{[^}]*@[PQEAa][^}]*\}/.test(expansionSource)
	...
	const parameterAssignmentWithEscapes =
		/\$\{[^}]*[=+\-?][^}]*\\[0-7]{3}[^}]*\}/.test(expansionSource) || // octal escapes
		/\$\{[^}]*[=+\-?][^}]*\\x[0-9a-fA-F]{2}[^}]*\}/.test(expansionSource) || // hex escapes
		/\$\{[^}]*[=+\-?][^}]*\\u[0-9a-fA-F]{4}[^}]*\}/.test(expansionSource) // unicode escapes
	...
	const indirectExpansion = /\$\{![^}]+\}/.test(expansionSource)

	// Check for here-strings with command substitution
	// <<<$(...) or <<<`...` can execute commands
	const hereStringWithSubstitution = /<<<\s*(\$\(|`)/.test(shellSyntaxSource)
	...
	const zshProcessSubstitution = /=\([^)]+\)/.test(shellSyntaxSource)
	...
	const zshGlobQualifier = /[*?+@!]\(e:[^:]+:\)/.test(shellSyntaxSource)
	...
	return (
		...
		zshGlobQualifier ||
		((process as any).platform === "win32" && /\^/.test(shellInterpretedSource)) // kilocode_change: Windows CMD specific escape character
	)
}

// kilocode_change start
function maskSingleQuotedSegmentsForSafetyScan(source: string): string {
	return maskQuotedSegments(source, { maskDoubleQuotes: false })
}

function maskQuotedSegmentsForShellSyntaxScan(source: string): string {
	return maskQuotedSegments(source, { maskDoubleQuotes: true })
}

function maskQuotedSegments(source: string, { maskDoubleQuotes }: { maskDoubleQuotes: boolean }): string {
	let result = ""
	let quote: "'" | '"' | undefined
	let escaped = false
	...
	for (const char of source) {
		...
		if (char === "\\") {
			result += quote && (quote === "'" || maskDoubleQuotes) ? " " : char
			escaped = quote !== "'"
			continue
		}
		...
		if (quote) {
			if (char === quote) {
				...
				result += quote === "'" || maskDoubleQuotes ? " " : char
			}
			continue
		}
...
function stripHeredocBodiesForSafetyScan(source: string): string {
	const lines = source.split(/\r\n|\r|\n/)
	const keptLines: string[] = []
	let pendingDelimiter: { marker: string; stripTabs: boolean; suppressesExpansion: boolean } | undefined

	for (const line of lines) {
		if (pendingDelimiter) {
			const candidate = pendingDelimiter.stripTabs ? line.replace(/^\t+/, "") : line

			if (!pendingDelimiter.suppressesExpansion) {
				keptLines.push(line)
			}

			if (candidate === pendingDelimiter.marker) {
				if (pendingDelimiter.suppressesExpansion) {
					keptLines.push(line)
				}
		...
		keptLines.push(line)
		const delimiter = extractFirstHeredocDelimiter(line)
		if (delimiter) {
...
function extractFirstHeredocDelimiter(
	line: string,
): { marker: string; stripTabs: boolean; suppressesExpansion: boolean } | undefined {
	const heredocPattern = /<<(-)?\s*(?:'([^']+)'|"([^"]+)"|(\\)?([^\s;&|()<>]+))/
	const match = heredocPattern.exec(line)
	const marker = match?.[2] ?? match?.[3] ?? match?.[5]

	return marker
		? {
				marker,
				stripTabs: match?.[1] === "-",
				suppressesExpansion: Boolean(match?.[2] ?? match?.[3] ?? match?.[4]),
			}
		: undefined
}
// kilocode_change end
...
export function isAutoApprovedSingleCommand(
	...
	const hasWildcard = allowedCommands.some((cmd) => cmd.trim().toLowerCase() === "*")
	...
	if (deniedCommands === undefined) {
		...
		return allowedCommands.some((prefix) => {
			const lowerPrefix = prefix.trim().toLowerCase()
			// Handle wildcard "*" - it matches any command
			return lowerPrefix === "*" || (!!lowerPrefix && trimmedCommand.startsWith(lowerPrefix))
		})
	}
