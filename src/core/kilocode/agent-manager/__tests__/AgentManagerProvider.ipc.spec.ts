...
describe("AgentManagerProvider IPC paths", () => {
	...
	it("sendMessage uses force-send IPC so busy tasks are interrupted before continuing", async () => {
		mockProcessHandler.hasStdin.mockReturnValue(true)
		mockProcessHandler.forceSendMessage.mockResolvedValue(undefined)

		await provider.sendMessage("sess", "hello")

		expect(mockProcessHandler.forceSendMessage).toHaveBeenCalledWith(
			"sess",
			expect.objectContaining({ type: "askResponse", askResponse: "messageResponse", text: "hello" }),
		)
		expect(mockProcessHandler.writeToStdin).not.toHaveBeenCalled()
	})

	it("sendMessage surfaces force-send IPC errors", async () => {
		mockProcessHandler.hasStdin.mockReturnValue(true)
		mockProcessHandler.forceSendMessage.mockRejectedValue(new Error("boom"))
