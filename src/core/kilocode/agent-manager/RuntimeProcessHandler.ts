...
interface ParentIPCMessage {
	type: "sendMessage" | "forceSendMessage" | "shutdown" | "injectConfig" | "resumeWithHistory" // kilocode_change
	payload?: unknown
}
...
export class RuntimeProcessHandler {
	...
	public sendMessage(sessionId: string, message: unknown): Promise<void> {
		return this.sendIpcMessage(sessionId, "sendMessage", message)
	}

	// kilocode_change start
	public forceSendMessage(sessionId: string, message: unknown): Promise<void> {
		return this.sendIpcMessage(sessionId, "forceSendMessage", message)
	}
	// kilocode_change end

	private sendIpcMessage(
		sessionId: string,
		type: ParentIPCMessage["type"],
		message: unknown,
	): Promise<void> {
		...
		this.callbacks.onLog(`[IPC→Agent] ${sessionId}: ${type}(${msgType}) ${msgText ? `"${msgText}..."` : ""}`)
