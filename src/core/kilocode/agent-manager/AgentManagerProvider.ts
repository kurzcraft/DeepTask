...
export class AgentManagerProvider implements vscode.Disposable {
	public static readonly viewType = "deeptask.AgentManagerPanel"
	...
	public async sendMessage(
		...
		this.appendUserFeedbackMessage(sessionId, content, images)
		// kilocode_change end

		// kilocode_change start
		// User chat input is preemptive: interrupt whatever the agent is doing, then
		// continue with the latest message. This avoids stale ask/queue states that
		// make the task-list chat appear unresponsive.
		const message = this.buildRuntimeMessage(content, images)
		await this.processHandler.forceSendMessage(sessionId, message)
		// kilocode_change end
	}

	// kilocode_change start
	private appendUserFeedbackMessage(sessionId: string, content: string, images?: string[]): void {
		if (!content && !images?.length) return

		const existingMessages = this.sessionMessages.get(sessionId) || []
		const hasDuplicate = existingMessages.some(
			(message) =>
				message.type === "say" &&
		...
		const updatedMessages = [
			...existingMessages,
			{
				...
				text: content,
				images,
				metadata: { agentManagerServerUserFeedback: true },
			} satisfies ClineMessage,
		]

		this.sessionMessages.set(sessionId, updatedMessages)
		this.postMessage({ type: "agentManager.chatMessages", sessionId, messages: updatedMessages })
	}
	// kilocode_change end
	...
	private async getHMRHtmlContent(webview: vscode.Webview): Promise<string> {
		...
		return /*html*/ `
			...
			<html lang="en">
				...
				<body>
					<div id="root" data-deeptask-icon-uri="${deeptaskIconUri}"></div>
					${reactRefreshScript}
