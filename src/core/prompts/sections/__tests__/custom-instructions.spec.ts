...
describe("addCustomInstructions", () => {
	...
	it("should include task progress file instructions when enabled by default", async () => {
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
		readFileMock.mockRejectedValue({ code: "ENOENT" })
		...
		expect(result).toContain("Canonical file format (mandatory; this is not YAML frontmatter)")
		expect(result).toContain("<!-- deeptask-task-id:<instance-id> -->")
		expect(result).toContain("example value is illustrative only and must never be copied")
		expect(result).toContain("current host task ID or runtime instance ID")
		expect(result).toContain("current host task ID is only a candidate-discovery key")
		expect(result).toContain("host restores the native list from the authoritative file")
		expect(result).toContain("do not rewrite the marker or repeat the same stale request")
		expect(result).toContain("Never add a `deeptask-native-todo-list` section")
		expect(result).toContain("exact top-to-bottom projection of every checklist item")
		expect(result).toContain("preserve order, text, status, and indentation-derived depth")
		expect(result).toContain("Keep one task's evolving checklist in its same Markdown file")
		expect(result).toContain("Every later executable request in the same conversation")
		expect(result).toContain("reuse the focused active file and its exact marker")
		expect(result).not.toContain("unique host-prefixed instance ID in the form host-task-id:work-instance-id")
	...
	it("should override other progress file instructions when disabled", async () => {
		...
		const result = await addCustomInstructions("", "Always create a progress checklist file.", "/fake/path", "", {
			settings: {
				maxConcurrentFileReads: 5,
				todoListEnabled: true,
				useAgentRules: false,
				newTaskRequireTodos: false,
				taskProgressFileEnabled: false,
			},
		})

		expect(result).not.toContain("Task Progress File:\nBefore completing any task")
		expect(result).toContain("Task Progress File Disabled:")
		expect(result).toContain("Do not create, update, or require Markdown progress checklist files")
	})

	it("should include task progress file instructions by default when no other instructions provided", async () => {
		// Simulate no .kilocode/rules directory
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
