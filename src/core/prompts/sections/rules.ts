...
- SCRIPT-FIRST COMMAND RULE: every command must be written to a script file under the current workspace's EXTRA/bash/ directory (create the directory when needed) and then executed with a short one-line command that runs only that script. Never send multi-line commands, heredocs, inline python -c / node -e programs, JSON/YAML/SQL payloads, or nested quoting layers directly to execute_command. Each script must stream the execution process live to the integrated terminal and persist complete stdout and stderr to a durable log file under the current workspace's EXTRA/output/ directory (for example with tee and pipefail on bash), printing the log file path and final exit status at the end. After execution, read the saved log with read_file instead of relying on terminal streaming. If terminal output is empty, truncated, delayed, or the completion status is uncertain, do not blindly rerun the operation; read the durable log first.
