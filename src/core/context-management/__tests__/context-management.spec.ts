...
describe("Context Management", () => {
	...
	describe("manageContext", () => {
		...
		it("should fall back to truncateConversation when autoCondenseContext is true but summarization fails", async () => {
			...
			expect(result.condenseError).toBe("Summarization failed")
			expect(result.prevContextTokens).toBe(totalTokens)
			// Should have all original messages + truncation marker
			expect(result.messages.length).toBe(6) // 5 original + 1 marker
		...
		it("should fall back to truncation when threshold condense fails before the allowed-token limit", async () => {
			const summarizeSpy = vi
				.spyOn(condenseModule, "summarizeConversation")
				.mockResolvedValue({
					messages,
					summary: "",
					cost: 0,
					error: "API stream timed out after 60000ms while waiting for the next chunk; no model output was received.",
				})

			const modelInfo = createModelInfo(100000, 30000)
			const totalTokens = 50001 // Above the 50% threshold but below allowedTokens (60000)
			const messagesWithSmallContent = [
				...messages.slice(0, -1),
				{ ...messages[messages.length - 1], content: "" },
			]
			...
			expect(result.messagesRemoved).toBe(2)
			expect(result.error).toBeUndefined()
			expect(result.condenseError).toContain("API stream timed out")

			summarizeSpy.mockRestore()
		})

		it("should fall back to truncateConversation when summarization throws", async () => {
			const summarizeSpy = vi
				.spyOn(condenseModule, "summarizeConversation")
				.mockRejectedValue(
					new Error(
			...
			expect(result.messagesRemoved).toBe(2)
			expect(result.error).toBeUndefined()
			expect(result.condenseError).toBe(
				"API stream timed out after 60000ms while waiting for the next chunk; no model output was received.",
			)
			expect(result.summary).toBe("")
		...
		it("should keep the summarization error when fallback truncation does not reduce context", async () => {
			const summarizeSpy = vi
				.spyOn(condenseModule, "summarizeConversation")
				.mockResolvedValue({
					messages,
			...
			const modelInfo = createModelInfo(1000, 100)
			const twoMessageHistory: ApiMessage[] = [
				{ role: "user", content: "hello" },
				{ role: "assistant", content: "world" },
			]

			const result = await manageContext({
				messages: twoMessageHistory,
				totalTokens: 950,
				contextWindow: modelInfo.contextWindow,
				maxTokens: modelInfo.maxTokens,
				apiHandler: mockApiHandler,
			...
			expect(result.truncationId).toBeDefined()
			expect(result.messagesRemoved).toBe(0)
			expect(result.error).toBe("provider 500: automatic summary failed")
