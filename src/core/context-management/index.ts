	...
	if (autoCondenseContext) {
		...
		if (contextPercent >= effectiveThreshold || prevContextTokens > allowedTokens) {
			...
			try {
				...
				console.error(`[manageContext] Failed to condense context: ${error}`)
			}
		}
	}

	// Fall back to sliding window truncation when the context is too large or
	// condensation failed. A failed summary must not leak into a normal API
	// request with the unchanged history.
	if (prevContextTokens > allowedTokens || !!error) {
		...
		return {
			...
			summary: "",
			cost,
			// kilocode_change: automatic condense provider errors are handled only when
			// sliding-window fallback actually removed messages and reduced context.
			// Otherwise surfacing a successful truncation would hide the real failure.
			error: fallbackReducedContext ? undefined : error,
			condenseError: error,
