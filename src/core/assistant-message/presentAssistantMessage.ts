...
export async function presentAssistantMessage(cline: Task) {
	...
	try {
			...
			case "tool_use": {
				...
				const toolDescription = (): string => {
					switch (block.name) {
						...
						case "dispatch_subagents": {
							...
							return `[${block.name}: ${count} parallel subagent(s)]`
						}
						case "workspace_status":
							return `[${block.name}]`
						case "workspace_create":
							return `[${block.name}: '${block.params.name ?? block.params.task_description ?? ""}']`
				...
				try {
					...
					if (block.name !== "browser_action") {
						await withToolExecutionTimeout(() => cline.browserSession.closeBrowser(), 10_000).catch(
							() => undefined,
						)
					}
				}
				...
				if (
					cline.shouldRejectToolUntilProgressListExpanded(block.name, block.params as Record<string, unknown>)
				) {
					cline.consecutiveMistakeCount++
					cline.recordToolError(block.name)
					pushToolResult(
						formatResponse.toolError(
							`Do not use ${block.name} yet. Your first tool call after a new user instruction must be update_todo_list to expand or replace the progress list for the new work. Do not restate prior completed items; create the new checklist first.`,
							toolProtocol,
						),
					)
				...
				// In a resumed/completed-task continuation, an immediate attempt_completion is
				// often a shortcut: the model acknowledges the new instruction and then
				// declares it done. Record only real work tools so completion is allowed after
				...
				await withToolExecutionTimeout(
					async () => {
						switch (block.name) {
							...
							// kilocode_change start: parallel subagents & workspaces
							case "dispatch_subagents":
								await dispatchSubagentsTool.handle(cline, block as ToolUse<"dispatch_subagents">, {
									askApproval,
									handleError,
									pushToolResult,
									removeClosingTag,
								...
								break
							case "workspace_status":
								await workspaceStatusTool.handle(cline, block as ToolUse<"workspace_status">, {
									askApproval,
									handleError,
									pushToolResult,
									removeClosingTag,
								...
								break
							case "workspace_create":
								await workspaceCreateTool.handle(cline, block as ToolUse<"workspace_create">, {
									askApproval,
									handleError,
									pushToolResult,
									removeClosingTag,
								...
								break
							case "workspace_merge":
								await workspaceMergeTool.handle(cline, block as ToolUse<"workspace_merge">, {
									askApproval,
									handleError,
									pushToolResult,
									removeClosingTag,
		...
		try {
			...
			console.error(`[Task#${cline.taskId}.${cline.instanceId}] Failed to present tool error`, presentationError)
		}

		const errorContent = formatResponse.toolError(
			failure.message,
			toolCallId ? TOOL_PROTOCOL.NATIVE : TOOL_PROTOCOL.XML,
		)
		if (toolCallId) {
...
async function withToolExecutionTimeout<T>(operation: () => Promise<T>, timeoutMs?: number, task?: Task): Promise<T> {
	...
	while (true) {
		...
		const outcome = await Promise.race([
			...
			new Promise<{ type: "tick" }>((resolve) =>
				setTimeout(() => resolve({ type: "tick" }), Math.min(100, remainingMs)),
			),
		])
