...
describe("Task reasoning preservation", () => {
	...
	it("should not persist plain streaming reasoning as API conversation content", async () => {
		const task = new Task({
			context: mockProvider.context as any,
			provider: mockProvider as ClineProvider,
			apiConfiguration: mockApiConfiguration,
		...
		// Mock API handler without provider-specific replayable reasoning data.
		task.api = {
			getModel: vi.fn().mockReturnValue({
				id: "test-model",
				info: {
		...
		expect(stored.role).toBe("assistant")
		expect(stored.content).toEqual([{ type: "text", text: assistantText }])
		expect(JSON.stringify(stored.content)).not.toContain(reasoningText)
	})
})
