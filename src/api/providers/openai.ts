...
export class OpenAiHandler extends BaseProvider implements SingleCompletionHandler {
	...
	protected addMaxTokensIfNeeded(
		...
		if (this.options.includeMaxTokens === true) {
			...
			// Negative values are sentinels for unknown/unlimited output and must not be sent to OpenAI-compatible APIs.
...
type OpenAiModelsResult = string[] & { modelInfos?: Record<string, ModelInfo> }
...
const getModelInfoFromOpenAiModel = (model: Record<string, any>): ModelInfo | undefined => {
	const contextWindow = getFirstPositiveNumber(
		model.contextWindow,
		...
		model.model_extra?.max_context_length,
		model.model_extra?.max_model_len,
	)
...
export async function getOpenAiModels(
	...
	try {
		...
		for (const model of models) {
			...
			const modelInfo = getModelInfoFromOpenAiModel(model)

			if (modelInfo) {
				modelInfos[model.id] = modelInfo
			}
		}

		if (Object.keys(modelInfos).length > 0) {
			Object.defineProperty(result, "modelInfos", {
				value: modelInfos,
				enumerable: false,
			})
		}
