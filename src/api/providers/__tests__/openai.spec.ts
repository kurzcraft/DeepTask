...
describe("OpenAiHandler", () => {
	...
	describe("createMessage", () => {
		...
		it("should omit max_tokens when includeMaxTokens is true but configured maxTokens is a sentinel", async () => {
			const optionsWithSentinelMaxTokens: ApiHandlerOptions = {
				...mockOptions,
				includeMaxTokens: true,
				openAiCustomModelInfo: {
			...
			const handlerWithSentinelMaxTokens = new OpenAiHandler(optionsWithSentinelMaxTokens)
			const stream = handlerWithSentinelMaxTokens.createMessage(systemPrompt, messages)
			for await (const _chunk of stream) {
			}
...
describe("getOpenAiModels", () => {
	...
	it("should expose model metadata from OpenAI-compatible API responses", async () => {
		const mockResponse = {
			data: {
				data: [
					{
		...
		expect(result.modelInfos?.["gpt-5.5"]).toEqual(
			expect.objectContaining({
