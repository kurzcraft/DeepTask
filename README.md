...
> **Deeptask 9.1.8** 前端双层控制看门狗 + 拒绝门断路器 + 取消灰死区自愈，彻底消灭"无按钮卡死"与"追问无回复"死角：点击"强制继续/恢复"后若宿主在宽限窗内无任何真实进展，界面强制渲染"继续 + 取消"兜底控制行；纯对话式追问（提问/确认/闲聊）的答复本身就是交付物，不再被"先干活再收尾"门误拒成死循环；同一续轮连续 3 次被拒后断路器强制放行收尾，用户必拿到回复；点击取消后若宿主卡死静默，死信看门狗强制渲染兜底控制行并自愈输入框；宿主侧取消操作 10 秒硬超时保证必广播状态刷新。
