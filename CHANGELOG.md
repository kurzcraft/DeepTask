...
- Fix the agent dead-loop where a follow-up user message after an interrupted tool never got any reply: the premature-completion rejection gate ("Do not claim the continued task is complete yet") now only applies to actionable work continuations. Pure conversational follow-ups (questions, acknowledgements, discussion) whose answer IS the deliverable are exempt, because they have no work tool to demand and the old gate trapped the model in an unfinishable turn.
- Add a rejection circuit breaker: after 3 consecutive premature-completion rejections in the same continuation turn the gate stands down and lets the completion through, so the model can always finish the turn with a reply instead of burning the mistake limit in a silent loop. Running any real work tool resets the breaker; each fresh user turn restores the full retry budget.
- Fix the grayed-out cancel dead zone: clicking Cancel/Terminate now arms a cancel-specific dead-letter watchdog — if the host stays silent past the grace window with streaming still frozen true, the fallback Proceed/Cancel row is force-rendered AND the composer self-heals (sendingDisabled/didClickCancel reset), so the user can always send a message or cancel again. A still-true isStreaming after a cancel click is treated as the stuck state itself, never as host progress.
- Host-side bounded cancel guarantee: `cancelTaskAndRestoreUi` races `cancelTask()` against a 10s hard deadline — abort/rehydration hangs can no longer swallow the final state broadcast, so the webview always regains its controls.
...
- Fix the first message after a task completion being silently ignored until a manual resend: the completion-boundary rewind chain parks the payload and then relies on cancel/rehydrate to consume it, but three branches (cancelTask's instanceId race-skip return, display-only liveElsewhere rehydration, and edit-resend routing into the stack-top background task) could return without ever consuming the parked message, leaving zero trace in storage. A parked human message now arms a 10s orphan-rescue guard that re-delivers it on the current task (or a rebuilt/fresh task as fallback) so every send after completion produces a model turn, and the completion-boundary rewind itself now targets the focused conversation's task.

## 9.1.6

### Patch Changes

- Fix the parallel-conversation running spinner disappearing when a finished task is reopened: `ensureTaskConversation` now clears a stale `completedAt` marker on reuse, the live-task broadcast no longer skips actively running tasks that still carry the marker, and the rail treats a running session as running even when the marker lingers.
- Fix the agent ignoring short negative user feedback ("没有用", "不行", "didn't work") after a repair attempt: the injected continuation guidance previously used a rigid two-way split (question/acknowledgement/discussion → conversational-only reply vs concrete work → milestones), which steered bare negative replies into the chat-only branch so the agent restated old status, repeated the failed approach, and asked the user to re-confirm it. The guidance is now semantic and chat-first: every user message must first get a natural conversational reply showing the agent understood it; tracking mechanisms (failure diagnosis, a materially different new approach, progress-list milestones) follow within the same reply instead of replacing it. The "supersedes" clause is bounded to earlier assistant turns and never overrides system rules. The reply is plain text in the same response as any tool calls (reply first, tools after); skipping the reply because a tool will be called, bare tool calls without words, and resuming interrupted tool work before replying are all forbidden.
