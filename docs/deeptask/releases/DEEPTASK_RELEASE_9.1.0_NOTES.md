...
- **左侧固定侧栏（文件夹 → 工作区 → 对话）**：聊天左侧新增 ZCode/DeepSeek harness 式宽侧栏，所有 VSCodium 窗口共享同一份全局文件夹列表。层级为文件夹、其下工作区、再下对话；打开窗口即从注册表与 `.kilocode/worktrees/` 加载已创建的工作区。占用只统计正在推理的对话；空闲对话可复用同一工作区。推理中模型可用 `workspace_merge` 自行切到空闲工作区或新建 worktree。删除工作区弹出原生确认：选择「是」强制删除 worktree 并把对话移回 main；选择「直接删除」同时删除该工作区的所有对话。
- **多窗口协同**：新窗口启动后自动同步各对话状态与集成终端；只要任一窗口仍有活动任务就不会停止推理，新任务会跨窗口同步。
...
- 文件：`deeptask-9.1.0.vsix`（40,501,760 字节）。
- SHA-256：`c5e29b70c603cf5088677f59ecc6d9827b2b7dd8fc60f116f2d804871104a513`。
