...
import { forceSendWebviewMessage } from "./force-send.js" // kilocode_change
			...
			try {
				switch (msg.type) {
					...
					case "forceSendMessage":
						if (msg.payload && agent) {
							logs.debug(`[Ext→] forceSendWebviewMessage(${payloadType})`, "AgentProcess")
							await forceSendWebviewMessage(agent, msg.payload as WebviewMessage)
						}
						break
					// kilocode_change end
